...
parser.add_argument("--exp", type=str, default="train_fixmatch_FixMatch")
parser.add_argument("--labeled-num", type=int, default=7)
...
def Inference():
    ...
    snapshot_path = (
        snapshot_path
    ) = f"exp/acdc/{FLAGS.exp}/{FLAGS.model}/{FLAGS.labeled_num}/{FLAGS.subfolder}"
    ...
    save_mode_path = os.path.join(snapshot_path, "best_purb.pth")
    temp_dict = torch.load(save_mode_path,weights_only=False)["model"]
